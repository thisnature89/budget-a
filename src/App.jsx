...
const initialData = { transactions: [], budgets: {}, fixedPayments: [] };
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{
      minHeight:"100vh",background:"#0b0f1a",display:"flex",alignItems:"center",justifyContent:"center",
      ...
      `}</style>
      <div style={{
        width:"100%",maxWidth:360,animation:"fadeUp 0.5s ease",
      }}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <p style={{color:"#64748b",fontSize:14,marginTop:8}}>로그인하고 시작하세요</p>
        </div>

        <div style={{background:"#111827",borderRadius:20,padding:24,border:"1px solid #1e293b"}}>
          <div style={{marginBottom:16}}>
            <label style={{display:"block",fontSize:12,fontWeight:600,color:"#94a3b8",marginBottom:6,letterSpacing:1}}>이메일</label>
          ...
          <div style={{marginBottom:20}}>
            <label style={{display:"block",fontSize:12,fontWeight:600,color:"#94a3b8",marginBottom:6,letterSpacing:1}}>비밀번호</label>
          ...
          {error && (
            <div style={{
              background:"#F43F5E18",border:"1px solid #F43F5E40",borderRadius:10,
              padding:"10px 14px",marginBottom:16,fontSize:13,color:"#F43F5E",
            }}>{error}</div>
          )}
          ...
          <button onClick={handleLogin} disabled={loading}
            style={{
              ...
              background: loading ? "#475569" : "linear-gradient(135deg, #FF6B35, #FF8F5E)",
...
export default function App() {
  ...
  const [fixedForm, setFixedForm] = useState({ name:"",principal:"",interest:"",payDay:"",endDate:"",memo:"" });
    ...
    return (
      <div style={{minHeight:"100vh",background:"#0b0f1a",display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{textAlign:"center",fontFamily:"'Noto Sans KR',sans-serif"}}>
          <div style={{width:32,height:32,border:"3px solid #1e293b",borderTop:"3px solid #FF6B35",borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 12px"}}/>
  ...
  function saveBudget(catId) {
    ...
    setData(d => ({ ...d, budgets: { ...d.budgets, [mk]: { ...(d.budgets[mk] || {}), [catId]: val } } }));
  ...
  function addFixedPayment() {
    ...
    const fp = { id: editingFixedId || Date.now().toString(), name, principal: Number(principal), interest: Number(interest || 0), payDay: pd, endDate, memo: fixedForm.memo };
    setData(d => {
      const existing = (d.fixedPayments || []);
      if (editingFixedId) return { ...d, fixedPayments: existing.map(x => x.id === editingFixedId ? fp : x) };
  ...
  function editFixed(fp) {
    setFixedForm({ name: fp.name, principal: String(fp.principal), interest: String(fp.interest), payDay: String(fp.payDay), endDate: fp.endDate, memo: fp.memo || "" });
  ...
  return (
    <div style={styles.root}>
        ...
        <div style={styles.headerInner}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={styles.monthNav}>
              ...
              <span style={styles.monthLabel}>{currentMonth.getFullYear()}년 {MONTHS[currentMonth.getMonth()]}</span>
              <button onClick={nextMonth} style={styles.navBtn}>→</button>
            </div>
            <button onClick={()=>signOut(auth)} style={{...styles.navBtn,fontSize:11,width:"auto",padding:"0 8px",color:"#64748b"}}>로그아웃</button>
          </div>
        </div>
      </div>
      ...
      <div style={styles.content}>
          ...
          <div className="slide-up">
            <div style={styles.summaryGrid}>
              <div style={{...styles.summaryCard, background:"linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)"}}>
                ...
                <span style={{fontSize:24,fontWeight:900,color:"#FF6B35",fontFamily:"'Space Mono',monospace",letterSpacing:"-1px"}}>{formatMoney(totalExpense)}</span>
                {totalBudget > 0 && (
                  <div style={{marginTop:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#64748b",marginBottom:4}}>
                      <span>예산 대비</span><span>{Math.round(totalExpense/totalBudget*100)}%</span>
                    </div>
                    <div style={{height:4,borderRadius:2,background:"#2a2a4a",overflow:"hidden"}}>
                      <div className="bar-fill" style={{height:"100%",width:`${Math.min(100,totalExpense/totalBudget*100)}%`,borderRadius:2,
              ...
              <div style={{...styles.summaryCard, background:"linear-gradient(135deg, #0f2027 0%, #203a43 100%)"}}>
                ...
                <span style={{fontSize:24,fontWeight:900,color:"#4ECDC4",fontFamily:"'Space Mono',monospace",letterSpacing:"-1px"}}>{formatMoney(totalIncome)}</span>
                <div style={{marginTop:10,fontSize:13,color: totalIncome - totalExpense >= 0 ? "#10B981" : "#F43F5E",fontWeight:600}}>잔액: {formatMoney(totalIncome - totalExpense)}</div>
              </div>
            </div>
            ...
            {(data.fixedPayments || []).length > 0 && (
              <div style={{marginBottom:20}}>
                <div style={{...styles.summaryCard, background:"linear-gradient(135deg, #1a1033 0%, #1e1145 100%)", border:"1px solid #312e81"}}>
                  ...
                  <span style={{fontSize:22,fontWeight:900,color:"#818CF8",fontFamily:"'Space Mono',monospace",letterSpacing:"-1px"}}>{formatMoney(fixedMonthlyTotal)}</span>
                  <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{(data.fixedPayments || []).length}건의 고정 지출</div>
                </div>
              </div>
            )}
            ...
            {byCategory.length === 0 ? (
              <div style={styles.empty}><span style={{fontSize:48,display:"block",marginBottom:12}}>🌱</span><span style={{color:"#64748b"}}>아직 이번 달 내역이 없어요</span></div>
            ) : (
                ...
                {byCategory.sort((a,b)=>(b.spent+b.fixedSum)-(a.spent+a.fixedSum)).map((cat,i) => (
                  <div key={cat.id} className="hover-lift" style={{...styles.catRow,animationDelay:`${i*0.06}s`}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:15,fontWeight:700,color:"#e2e8f0",fontFamily:"'Space Mono',monospace"}}>{formatMoney(cat.spent + cat.fixedSum)}</div>
                      {cat.budget > 0 && (
        ...
        {view === "list" && (
          <div className="slide-up">
            <div style={styles.sectionTitle}>거래 내역<span style={{fontSize:13,color:"#64748b",fontWeight:400,marginLeft:8}}>{monthTransactions.length}건</span></div>
            {monthTransactions.length === 0 ? (
              <div style={styles.empty}><span style={{fontSize:48,display:"block",marginBottom:12}}>📝</span><span style={{color:"#64748b"}}>내역이 없어요. 아래 + 버튼으로 추가해보세요!</span></div>
            ) : (
                ...
                {[...monthTransactions].sort((a,b)=>b.date.localeCompare(a.date)||b.id-a.id).map((tx,i) => {
                  ...
                  return (
                    <div key={tx.id} className="slide-up hover-lift" style={{...styles.txRow,animationDelay:`${i*0.04}s`}}>
                      <div style={{width:38,height:38,borderRadius:10,background: tx.type==="income" ? "#4ECDC418" : `${cat.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>
                        {tx.type==="income" ? "💵" : cat.icon}
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:13,fontWeight:600,color:"#e2e8f0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tx.memo || (tx.type==="income" ? "수입" : cat.name)}</div>
                        <div style={{fontSize:11,color:"#64748b",marginTop:2}}>{tx.date.slice(5).replace("-","/")} · {cat.name}</div>
                      </div>
                      <div style={{textAlign:"right",display:"flex",alignItems:"center",gap:8}}>
                        <span style={{fontSize:14,fontWeight:700,fontFamily:"'Space Mono',monospace",color: tx.type==="income" ? "#4ECDC4" : "#FF6B35"}}>
        ...
        {view === "fixed" && (
          <div className="slide-up">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              ...
              <button onClick={()=>{setShowAddFixed(true);setEditingFixedId(null);setFixedForm({name:"",principal:"",interest:"",payDay:"",endDate:"",memo:""});}}
            ...
            <div style={{fontSize:13,color:"#64748b",marginBottom:16}}>매월 고정으로 나가는 대출금·이자를 관리하세요</div>

            {(data.fixedPayments || []).length > 0 && (
              <div style={{background:"linear-gradient(135deg, #1a1033 0%, #1e1145 100%)",borderRadius:16,padding:16,marginBottom:16,border:"1px solid #312e81"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                  <div>
                    <div style={{fontSize:12,color:"#a5b4fc",fontWeight:500,marginBottom:4}}>월 납부 총액</div>
                    <div style={{fontSize:24,fontWeight:900,color:"#818CF8",fontFamily:"'Space Mono',monospace"}}>{formatMoney(fixedMonthlyTotal)}</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:12,color:"#a5b4fc",fontWeight:500,marginBottom:4}}>총 원금 / 이자</div>
                    <div style={{fontSize:14,fontWeight:700,color:"#c4b5fd",fontFamily:"'Space Mono',monospace"}}>{formatMoney((data.fixedPayments||[]).reduce((s,f)=>s+f.principal,0))}</div>
                    <div style={{fontSize:12,color:"#f0abfc",marginTop:2,fontFamily:"'Space Mono',monospace"}}>+ {formatMoney((data.fixedPayments||[]).reduce((s,f)=>s+f.interest,0))}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {(data.fixedPayments || []).length === 0 ? (
              <div style={styles.empty}><span style={{fontSize:48,display:"block",marginBottom:12}}>🏦</span><span style={{color:"#64748b"}}>등록된 고정납부가 없어요</span><div style={{fontSize:12,color:"#475569",marginTop:8}}>위의 + 추가 버튼으로 등록해보세요</div></div>
            ) : (
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {(data.fixedPayments || []).map((fp, i) => {
                  ...
                  const monthsRemain = monthsBetween(`${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}-01`, fp.endDate);
                  ...
                  return (
                    <div key={fp.id} className="slide-up" style={{...styles.budgetRow,animationDelay:`${i*0.06}s`,
                      border: isUrgent ? "1px solid #F43F5E60" : isExpired ? "1px solid #47556840" : "1px solid #312e81",opacity: isExpired ? 0.5 : 1}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                        <div style={{background:"#0f172a",borderRadius:10,padding:"10px 12px"}}>
                          ...
                          <div style={{fontSize:14,fontWeight:700,color:"#c4b5fd",fontFamily:"'Space Mono',monospace"}}>{formatMoney(fp.principal)}</div>
                        </div>
                        <div style={{background:"#0f172a",borderRadius:10,padding:"10px 12px"}}>
                          <div style={{fontSize:10,color:"#64748b",fontWeight:600,marginBottom:3}}>월 이자</div>
                          <div style={{fontSize:14,fontWeight:700,color:"#f0abfc",fontFamily:"'Space Mono',monospace"}}>{formatMoney(fp.interest)}</div>
                        </div>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12}}>
                        <div style={{display:"flex",gap:12}}>
                          <span style={{color:"#94a3b8"}}>📅 매월 <span style={{fontWeight:700,color:"#e2e8f0"}}>{fp.payDay}일</span></span>
                          <span style={{color:"#94a3b8"}}>⏳ 만기 <span style={{fontWeight:600,color:"#e2e8f0"}}>{fp.endDate.slice(0,7).replace("-",".")}</span></span>
                        </div>
                      </div>
                      {!isExpired && (
                        <div style={{marginTop:10,background:"#0f172a",borderRadius:10,padding:"10px 12px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <div>
                              ...
                              <div style={{fontSize:13,fontWeight:600,color:"#e2e8f0"}}>{monthsRemain > 0 ? `${monthsRemain}개월` : "이번 달 만기"}</div>
                            </div>
                            <div style={{textAlign:"right"}}>
                              <div style={{fontSize:10,color:"#64748b",fontWeight:600,marginBottom:2}}>남은 총 금액</div>
                              <div style={{fontSize:15,fontWeight:900,color:"#818CF8",fontFamily:"'Space Mono',monospace"}}>{formatMoney(remainTotal)}</div>
                            </div>
                          </div>
                          {(() => {
                            ...
                            return (
                              <div style={{marginTop:8}}>
                                <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#475569",marginBottom:3}}>
                                  <span>상환 진행률</span><span>{Math.round(pct)}%</span>
                                </div>
        ...
        {view === "budget" && (
          <div className="slide-up">
            ...
            <div style={{fontSize:13,color:"#64748b",marginBottom:16}}>각 카테고리를 눌러 예산을 설정하세요</div>

            {fixedMonthlyTotal > 0 && (
              <div style={{background:"#1e114520",border:"1px solid #312e8140",borderRadius:12,padding:"10px 14px",marginBottom:14,fontSize:12,color:"#a5b4fc",display:"flex",alignItems:"center",gap:8}}>
                <span>🏦</span><span>고정납부 월 {formatMoney(fixedMonthlyTotal)}이 대출/이자 카테고리에 자동 합산됩니다</span>
              </div>
            )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {CATEGORIES.map((cat,i) => {
                ...
                return (
                  <div key={cat.id} className="slide-up" style={{...styles.budgetRow,animationDelay:`${i*0.05}s`,
                    border: isEditing ? `1px solid ${cat.color}60` : "1px solid #1e293b"}}>
                    <div onClick={()=>{ if (!isEditing) { setEditingBudget(cat.id); setBudgetInput(budget > 0 ? String(budget) : ""); } }} style={{cursor:"pointer"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                        <span style={{fontSize:22}}>{cat.icon}</span>
                        <span style={{fontSize:14,fontWeight:600,color:"#e2e8f0",flex:1}}>{cat.name}</span>
                        <div style={{textAlign:"right"}}>
                          <span style={{fontSize:13,fontWeight:700,color:cat.color,fontFamily:"'Space Mono',monospace"}}>{formatMoney(totalSpent)}</span>
                          {fixedAdd > 0 && <div style={{fontSize:10,color:"#818CF8",marginTop:1}}>고정 {formatMoney(fixedAdd)} 포함</div>}
                        </div>
                      </div>
                      {budget > 0 && !isEditing && (
                        <div>
                      ...
                      {budget === 0 && !isEditing && <div style={{fontSize:12,color:"#475569"}}>탭하여 예산 설정</div>}
                    </div>
                    {isEditing && (
                      <div className="fade-in" style={{display:"flex",gap:8,marginTop:8}}>
                        <input type="number" placeholder="예산 금액 (원)" value={budgetInput} onChange={e=>setBudgetInput(e.target.value)} autoFocus style={styles.input} onKeyDown={e=>{ if(e.key==="Enter") saveBudget(cat.id); }}/>
        ...
        <div className="fade-in" style={styles.overlay} onClick={()=>setShowAdd(false)}>
          <div className="slide-up" style={styles.modal} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",gap:4,marginBottom:16,background:"#0f172a",borderRadius:12,padding:4}}>
              {[{v:"expense",l:"지출"},{v:"income",l:"수입"}].map(t => (
                <button key={t.v} onClick={()=>setForm(f=>({...f,type:t.v}))}
                  style={{flex:1,padding:"10px 0",borderRadius:10,border:"none",fontSize:14,fontWeight:600,cursor:"pointer",transition:"all 0.2s",fontFamily:"'Noto Sans KR',sans-serif",
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
                  {CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={()=>setForm(f=>({...f,category:cat.id}))}
                      style={{padding:"10px 4px",borderRadius:10,border:"none",cursor:"pointer",transition:"all 0.15s",fontFamily:"'Noto Sans KR',sans-serif",
                        background: form.category===cat.id ? `${cat.color}25` : "#0f172a",border: form.category===cat.id ? `2px solid ${cat.color}` : "2px solid transparent"}}>
            ...
            <div style={{marginBottom:14}}>
              ...
              <input type="text" placeholder="간단한 메모 (선택)" value={form.memo} onChange={e=>setForm(f=>({...f,memo:e.target.value}))} style={styles.input}/>
            </div>
            <div style={{marginBottom:20}}>
              <label style={styles.label}>날짜</label>
              <input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))} style={{...styles.input,colorScheme:"dark"}}/>
            </div>
      ...
      {showAddFixed && (
        <div className="fade-in" style={styles.overlay} onClick={()=>{setShowAddFixed(false);setEditingFixedId(null);}}>
          <div className="slide-up" style={styles.modal} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:18,fontWeight:700,color:"#e2e8f0",marginBottom:20}}>{editingFixedId ? "고정납부 수정" : "고정납부 추가"}</div>
            <div style={{marginBottom:14}}>
              <label style={styles.label}>대출명 *</label>
              <input type="text" placeholder="예: 주택담보대출" value={fixedForm.name} onChange={e=>setFixedForm(f=>({...f,name:e.target.value}))} autoFocus style={styles.input}/>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              <div><label style={styles.label}>월 원금 *</label><input type="number" placeholder="0" value={fixedForm.principal} onChange={e=>setFixedForm(f=>({...f,principal:e.target.value}))} style={styles.input}/></div>
              <div><label style={styles.label}>월 이자</label><input type="number" placeholder="0" value={fixedForm.interest} onChange={e=>setFixedForm(f=>({...f,interest:e.target.value}))} style={styles.input}/></div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              <div><label style={styles.label}>납부일 (매월) *</label><input type="number" placeholder="25" min="1" max="31" value={fixedForm.payDay} onChange={e=>setFixedForm(f=>({...f,payDay:e.target.value}))} style={styles.input}/></div>
              <div><label style={styles.label}>만기일 *</label><input type="date" value={fixedForm.endDate} onChange={e=>setFixedForm(f=>({...f,endDate:e.target.value}))} style={{...styles.input,colorScheme:"dark"}}/></div>
            </div>
            <div style={{marginBottom:20}}>
              <label style={styles.label}>메모</label>
              <input type="text" placeholder="은행명, 금리 등 (선택)" value={fixedForm.memo} onChange={e=>setFixedForm(f=>({...f,memo:e.target.value}))} style={styles.input}/>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{setShowAddFixed(false);setEditingFixedId(null);}} style={{...styles.btnGhost,flex:1}}>취소</button>
              <button onClick={addFixedPayment} style={{...styles.btnPrimary,flex:2}}>{editingFixedId ? "수정하기" : "추가하기"}</button>
            </div>
          </div>
        </div>
      )}
...
const styles = {
  root: { fontFamily:"'Noto Sans KR',sans-serif", background:"#0b0f1a", minHeight:"100vh", color:"#e2e8f0", position:"relative", maxWidth:480, margin:"0 auto", paddingBottom:100 },
  ...
  logo: { fontSize:20, fontWeight:900, background:"linear-gradient(135deg, #FF6B35, #FFB347)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", letterSpacing:"-0.5px" },
  monthNav: { display:"flex", alignItems:"center", gap:8 },
  navBtn: { width:32, height:32, borderRadius:8, border:"1px solid #1e293b", background:"#111827", color:"#94a3b8", fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Noto Sans KR',sans-serif" },
  ...
  tab: { flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2, padding:"8px 0", borderRadius:10, border:"none", background:"transparent", color:"#64748b", cursor:"pointer", transition:"all 0.2s", fontFamily:"'Noto Sans KR',sans-serif" },
  tabActive: { background:"#1e293b", color:"#FF6B35" },
  content: { padding:"0 20px" },
  summaryGrid: { display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 },
  summaryCard: { borderRadius:16, padding:16, display:"flex", flexDirection:"column", gap:6, border:"1px solid #1e293b" },
  sectionTitle: { fontSize:16, fontWeight:700, color:"#e2e8f0", marginBottom:12 },
  catRow: { background:"#111827", borderRadius:14, padding:"12px 14px", display:"flex", alignItems:"center", border:"1px solid #1e293b" },
  txRow: { background:"#111827", borderRadius:12, padding:"10px 12px", display:"flex", alignItems:"center", gap:10, border:"1px solid #1e293b" },
  budgetRow: { background:"#111827", borderRadius:14, padding:14, border:"1px solid #1e293b", transition:"all 0.2s" },
  empty: { textAlign:"center", padding:"48px 20px", color:"#475569", fontSize:14 },
  fab: { position:"fixed", bottom:28, right:"calc(50% - 220px)", width:56, height:56, borderRadius:16, border:"none", background:"linear-gradient(135deg, #FF6B35, #FF8F5E)", color:"#fff", fontSize:28, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 8px 30px rgba(255,107,53,0.35)", transition:"transform 0.2s ease", zIndex:30 },
  ...
  modal: { background:"#1a1f2e", borderRadius:24, padding:24, width:"100%", maxWidth:440, maxHeight:"85vh", overflow:"auto", border:"1px solid #2a3040" },
  label: { display:"block", fontSize:12, fontWeight:600, color:"#94a3b8", marginBottom:6, textTransform:"uppercase", letterSpacing:1 },
  input: { width:"100%", padding:"12px 14px", borderRadius:10, border:"1px solid #2a3040", background:"#0f172a", color:"#e2e8f0", fontSize:14, fontFamily:"'Noto Sans KR',sans-serif", transition:"border-color 0.2s" },
  btnPrimary: { padding:"12px 20px", borderRadius:12, border:"none", background:"linear-gradient(135deg, #FF6B35, #FF8F5E)", color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"'Noto Sans KR',sans-serif" },
  btnGhost: { padding:"12px 20px", borderRadius:12, border:"1px solid #2a3040", background:"transparent", color:"#94a3b8", fontSize:15, fontWeight:500, cursor:"pointer", fontFamily:"'Noto Sans KR',sans-serif" },
  deleteBtn: { width:24, height:24, borderRadius:6, border:"none", background:"transparent", color:"#475569", fontSize:12, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.15s", fontFamily:"'Noto Sans KR',sans-serif" },
  toast: { position:"fixed", bottom:96, left:"50%", transform:"translateX(-50%)", background:"#1e293b", color:"#e2e8f0", padding:"10px 20px", borderRadius:12, fontSize:13, fontWeight:500, boxShadow:"0 8px 30px rgba(0,0,0,0.3)", zIndex:200, animation:"toastIn 0.3s ease", border:"1px solid #2a3040" },
};
